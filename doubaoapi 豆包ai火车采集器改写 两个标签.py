...
#以下是用户编写代码区域
    if(PageType=="Save"):
        ...
        o_content1 = LabelArray['采集内容1']
        o_content2 = LabelArray['采集内容2'] 
        api_key = LabelArray['api_key'] 
        model = LabelArray['model'] 
        prompt1 = LabelArray['prompt1']
        prompt2 = LabelArray['prompt1'] 
        
        result1 = get_doubao_api(api_key, model, prompt1+o_content1)
        result2 = get_doubao_api(api_key, model, prompt2+o_content2)

        LabelArray['改写内容1'] = result1
        LabelArray['改写内容2'] = result2
    else:
